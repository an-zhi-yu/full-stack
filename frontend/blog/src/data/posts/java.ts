...
const javaPosts: Post[] = [
  ...
  // ─────────────────────────────────────────────────────────
  {
    ...
    pinned: true,
    content: [
      { type: 'paragraph', text: 'Java 是强类型语言——每个变量在声明时就必须明确指定类型，编译器会在编译阶段做类型检查。这与 JavaScript 的动态类型有本质区别，但与 TypeScript 非常相似。' },
      { type: 'heading', level: 2, text: '一、变量的定义与赋值', anchor: 'variables' },
      { type: 'paragraph', text: '变量是"有名字的一块内存"，用来存储数据，之后可以修改。定义格式：数据类型 变量名 = 值;' },
      { type: 'code', lang: 'java', caption: '变量定义的两种写法', code: `// 定义并赋初值（推荐）
int age = 18;
String name = "张三";
...
double price;
price = 99.5;` },
      { type: 'tip', title: '与前端对比', text: '前端 let age = 18 不需要类型；Java 必须写 int age = 18。TypeScript 的 const age: number = 18 与 Java 最像。' },
      { type: 'heading', level: 2, text: '二、基本数据类型（8种）', anchor: 'primitive' },
      { type: 'paragraph', text: 'Java 有 8 种基本类型，变量直接存储值本身（不是对象引用），性能更高。' },
      {
        type: 'table',
      ...
      { type: 'code', lang: 'java', caption: '8 种基本类型示例', code: `// 整数
byte  b = 127;
short s = 30000;
...
char c  = 'A';
char ch = '中';` },
      { type: 'warning', title: '常见坑', text: 'Java 的 boolean 不能像 JS 一样用 0 / "" / null 表示 false。if (1) 会报编译错误，条件必须是 boolean 类型。' },
      { type: 'heading', level: 2, text: '三、引用类型', anchor: 'reference' },
      { type: 'paragraph', text: '引用类型的变量存储的是"对象在堆内存中的地址"，而不是值本身。最常用的是 String 和数组。' },
      { type: 'code', lang: 'java', caption: '引用类型：String 与数组', code: `// String：双引号，S 大写（String 是类）
String name = "张三";
...
// Main.main({ "你好" });                  // ❌ 编译错误` },
      { type: 'tip', title: '与前端对比', text: 'JS 的 const arr = [1, 2, 3] 对应 Java 的 int[] arr = {1, 2, 3}；JS 的 const s = "hello" 对应 Java 的 String s = "hello"（注意 String 大写）。' },
      { type: 'heading', level: 2, text: '四、类型转换', anchor: 'casting' },
      { type: 'code', lang: 'java', caption: '自动转换 vs 强制转换', code: `// 自动类型转换（小 → 大，不丢失精度）
int   a = 100;
...
int    n  = Integer.parseInt("42"); // String → int` },
  ...
  // ─────────────────────────────────────────────────────────
  {
    ...
    readTime: 7,
    content: [
      { type: 'paragraph', text: '控制流决定了程序"按什么顺序"执行代码。Java 的 if/for/while 语法与 JavaScript 高度相似，但有一些值得注意的差异。' },
      { type: 'heading', level: 2, text: '一、if / else if / else', anchor: 'if' },
      { type: 'code', lang: 'java', caption: 'if-else 基本结构', code: `int score = 85;
...
if (score >= 90) {
    ...
    System.out.println("不及格");
      ...
      { type: 'warning', title: '注意', text: 'Java 的 if 条件必须是 boolean 类型，不能写 if (1) 或 if ("") — 这在 JavaScript 里合法，但 Java 编译直接报错。' },
      { type: 'heading', level: 2, text: '二、for 循环（三种写法）', anchor: 'for' },
      { type: 'code', lang: 'java', caption: '普通 for / for-each / 索引遍历', code: `int[] nums = { 10, 20, 30 };
...
for (String w : words) {
    System.out.println(w.toLowerCase());
      ...
      { type: 'tip', title: '选哪种？', text: '只需遍历元素 → 用 for-each（简洁）；需要索引或修改元素 → 用普通 for。' },
      { type: 'heading', level: 2, text: '三、while 与 do-while', anchor: 'while' },
      { type: 'code', lang: 'java', caption: 'while vs do-while', code: `// while：先判断，再执行（条件一开始就不满足则一次都不执行）
int n = 0;
while (n < 3) {
...
do {
    ...
    m++;
      ...
      { type: 'code', lang: 'java', caption: 'break 与 continue 示例', code: `// break：跳出整个循环
for (int i = 0; i < 10; i++) {
    if (i == 5) break;
...
for (int i = 0; i < 5; i++) {
    ...
    System.out.println(i);    // 输出 0 1 3 4
      ...
      { type: 'code', lang: 'java', caption: 'switch 语句（Java 14+ 新写法）', code: `// 传统写法（需要 break 防止穿透）
int day = 3;
switch (day) {
...
String result = switch (day) {
  ...
  // ─────────────────────────────────────────────────────────
  {
    ...
    pinned: true,
    content: [
      { type: 'paragraph', text: 'Java 数组是固定长度、同类型元素的有序集合。与 JavaScript 的数组相比，Java 数组一旦创建长度就不能改变；需要动态长度时应使用 List。' },
      { type: 'heading', level: 2, text: '一、创建与访问', anchor: 'create' },
      { type: 'code', lang: 'java', caption: '三种创建方式', code: `// ① 声明 + 赋初值（简写，仅限此处）
int[] nums = { 3, 1, 4, 1, 5 };
...
nums[0] = 99;                     // 修改` },
      ...
      { type: 'code', lang: 'java', caption: 'Arrays 常用 API', code: `import java.util.Arrays;
...
boolean same = Arrays.equals(nums, copy);         // false` },
      ...
      { type: 'code', lang: 'java', caption: 'Arrays.asList 快速构造 List', code: `import java.util.Arrays;
...
// ⚠️ asList 返回的 List 大小固定，不能 add / remove` },
      ...
      { type: 'paragraph', text: 'Stream 是 Java 8 引入的函数式 API，与前端的 Array.filter/map/reduce 思路完全一致。' },
      { type: 'code', lang: 'java', caption: 'filter / map / anyMatch / allMatch', code: `import java.util.Arrays;
import java.util.List;
...
long cnt = Arrays.stream(intArr).filter(x -> x > 2).count(); // 3` },
      { type: 'tip', title: '工具类模式', text: '工具方法（如求最大最小值）应放在独立工具类里（如 ArrayUtils），方法设为 public static，用类名调用：ArrayUtils.findMax(arr)。这与前端的 utils.ts 是同一个思路。' },
    ],
  },
  ...
  // ─────────────────────────────────────────────────────────
  {
    ...
    readTime: 9,
    content: [
      { type: 'paragraph', text: 'Java 的 String 是不可变对象（immutable）——每次"修改"字符串都会产生新对象，原字符串不变。这和 JavaScript 中字符串的行为一致，但在 Java 中更需要注意性能问题。' },
      { type: 'heading', level: 2, text: '一、常用查询 API', anchor: 'query' },
      { type: 'code', lang: 'java', caption: '查询方法', code: `String s = "Hello, Java!";
...
s.endsWith("!")         // true` },
      ...
      { type: 'code', lang: 'java', caption: '截取、替换、拼接', code: `String s = "Hello, Java!";
...
String.join("-", "a", "b", "c")            // "a-b-c"` },
      ...
      { type: 'warning', title: '高频坑：不能用 == 比较字符串内容', text: '== 比较的是对象引用地址，不是内容。比较内容必须用 equals()。' },
      { type: 'code', lang: 'java', caption: '正确的字符串比较', code: `String a = new String("hello");
...
String msg = String.format("你好，%s！今年 %d 岁。", "张三", 18);` },
      { type: 'heading', level: 2, text: '四、StringBuilder（大量拼接时用）', anchor: 'string-builder' },
      { type: 'paragraph', text: '循环里用 + 拼接字符串会产生大量临时对象，性能差。应用 StringBuilder，它在内部用可变缓冲区，拼接完再 toString() 得到字符串。' },
      { type: 'code', lang: 'java', caption: 'StringBuilder 用法', code: `// 链式写法（推荐）
...
sb.reverse();            // 反转` },
  ...
  // ─────────────────────────────────────────────────────────
  {
    ...
    content: [
      ...
      { type: 'code', lang: 'java', caption: '算术与赋值运算符', code: `int a = 10, b = 3;
...
a += 5;  a *= 2;` },
      ...
      { type: 'paragraph', text: '基本类型不是对象，不能直接放入泛型集合（List<Integer>）。Java 为每种基本类型提供了对应的包装类（Wrapper Class）。' },
      {
        type: 'table',
      ...
      { type: 'code', lang: 'java', caption: '包装类与自动装箱/拆箱', code: `// String ↔ 数字
...
x == y;       // false（> 127 时不复用缓存）` },
      ...
      { type: 'code', lang: 'java', caption: 'Math 常用方法速查', code: `Math.max(3, 7)      // 7
Math.min(3, 7)      // 3
Math.abs(-5)        // 5
...
int rand = (int)(Math.random() * (max - min + 1)) + min;` },
  ...
  // ─────────────────────────────────────────────────────────
  {
    ...
    pinned: true,
    content: [
      { type: 'paragraph', text: '面向对象编程（OOP）的核心思想是用"类"描述一类事物，用"对象"代表具体实例。Java 从底层就是面向对象的语言（除了基本类型外，一切都是对象）。' },
      { type: 'heading', level: 2, text: '一、类与对象', anchor: 'class-object' },
      { type: 'code', lang: 'java', caption: '定义一个 Book 类', code: `public class Book {
    ...
    public String toString() {
      ...
      { type: 'code', lang: 'java', caption: '使用 Book 对象', code: `Book b1 = new Book("Java 核心技术", "Horstmann", 129.0);
...
// b2.setPrice(-1);  // 抛出 IllegalArgumentException` },
      { type: 'tip', title: '与前端类对比', text: 'JS/TS 的 class Book { constructor(...) {} } 结构上与 Java 一致。主要差异：Java 的 private 由编译器强制执行；TS 的 private 只在编译时约束，运行时仍可访问。' },
      ...
      { type: 'code', lang: 'java', caption: '参数列表不同即可重载', code: `public class Person {
    private String name;
    private int    age;
...
Person p3 = new Person();           // name="匿名", age=0` },
  ...
  // ─────────────────────────────────────────────────────────
  {
    ...
    readTime: 10,
    content: [
      { type: 'paragraph', text: 'Java 中的方法分为三种主要类型：构造方法、实例方法、静态方法。搞清楚它们的区别是理解 Java OOP 的关键。' },
      ...
      { type: 'code', lang: 'java', caption: '四种方法写法对照', code: `public class MethodDemo {
    ...
    public static void main(String[] args) {
      ...
      { type: 'heading', level: 2, text: '三、static 与 public 的区别', anchor: 'static-vs-public' },
      {
        type: 'table',
      ...
      { type: 'tip', title: '记忆口诀', text: 'static → 管"要不要 new"；public/private → 管"谁能用"。两者独立，可任意组合：public static（工具方法）、private static（内部工具）、public void（实例方法）。' },
      { type: 'heading', level: 2, text: '四、工具类模式', anchor: 'utils' },
      { type: 'paragraph', text: '工具类（Utils）专门放 public static 方法，不需要实例化。与前端的 utils.ts 是同一思路。' },
      { type: 'code', lang: 'java', caption: 'ArrayUtils 工具类示例', code: `public class ArrayUtils {
...
ArrayUtils.findMin(nums); // 1` },
      ...
      { type: 'code', lang: 'java', caption: '包的声明与 import', code: `// 文件顶部声明所属包
...
public class Main {
    public static void main(String[] args) {
      ...
      { type: 'tip', title: '默认包', text: '如果文件顶部没有 package 语句，类就在"默认包"里。同一目录下同为默认包的类可以直接互相调用，不需要 import，这就是 ArrayDemo 可以直接写 ArrayUtils.findMax(arr) 的原因。' },
    ],
  },
]
