...
public record PostUpsertRequestDTO(
                @NotNull(message = "userId 不能为 null") Long userId,
                ...
                @NotBlank(message = "categorySlug 不能为空") String categorySlug,

                @NotNull(message = "tags 不能为 null") List<String> tags,

                @NotBlank(message = "date 不能为空") String date,

                @Min(value = 0, message = "readTime 不能为负") int readTime,

                Boolean pinned,

                @NotNull(message = "content 不能为 null") List<PostContent> content) {
}
